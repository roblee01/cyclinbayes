...
Then using the posterior samples, we can quantify uncertainty directly from them. For every parameter, we have a full distribution, where we can compute posterior means, standard deviations, and from that credible intervals. Then for the edges we can compute posterior inclusion probabilities. Below we have an example of analyzing the posterior inclusion probabilities for each of the possible edges in the graph.

<div style="display: flex; gap: 20px; align-items: flex-start;"> <div>
```{r,echo=FALSE}
knitr::kable(Adjacency_matrix_true|>kable_styling(font_size = 10))
```
</div> <div>
```{r,echo=FALSE}
Adjacency_matrix_means = matrix(colMeans(Adjacency_matrix_list[(0.75 * num_iter):num_iter,]),num_covariates,num_covariates)
knitr::kable(Adjacency_matrix_means|>kable_styling(font_size = 10))
```
</div> </div>

Each of the entries in the matrix on the right provides the probability that the edge is present in the graph. Below we plot the credible intervals for each of the present causal effect coefficients, where the red line represents the true causal weights. As seen with the provided credible intervals for the non zero causal weights, each credible interval is close to the true red line. Therefore, this indicates the method can both accurately determine causal weights and structure. 

```{r}
num_causal = length(which(c(Adjacency_matrix_true)==1))
Causal_effect_matrix_non_zero = results_list$Causal_effect_matrix_list[(0.75*num_iter):num_iter,which(c(Adjacency_matrix_true)==1)]
x = 1:num_causal
#column_means = colMeans(Causal_effect_matrix_non_zero)

#plot(x,column_means)


confint_matrix = matrix(0,num_causal,2)


for(i in 1:num_causal){
  confint_matrix[i,] = quantile(Causal_effect_matrix_non_zero[,i],c(0.025,0.975))
}
data = cbind(x,column_means)

ggplot(data,aes(x = x, y = column_means))+
  geom_point(size = 3) +  # points
  geom_hline(yintercept = 1, linetype = "dashed", color = "red")+
  geom_errorbar(aes(ymin = confint_matrix[,1], ymax = confint_matrix[,2]), width = 0.2) +  # error bars
...
Below we again plot the credible intervals for each of the present causal effect coefficients, where the red line represents the true causal weights. As seen with the provided credible intervals for the non zero causal weights, each credible interval is close to the true red line. Therefore, this indicates the method can both accurately determine causal weights and structure. 
...
```{r,echo=FALSE}
#matrix(colMeans(Adjacency_matrix_list[(0.75*num_iter):num_iter,]),num_covariates,num_covariates)
```
