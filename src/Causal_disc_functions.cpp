...
arma::mat posterior_mean_adjacency(arma::mat Adjacency_matrix_samples, double num_features){
  ...
  int num_iter = Adjacency_matrix_samples.n_rows;
  int start_row = std::round(0.85 * num_iter - 1);

  arma::rowvec Adjacency_matrix_means = arma::mean(Adjacency_matrix_samples.rows(start_row, num_iter - 1), 0);

  arma::mat mean_matrix = arma::reshape(Adjacency_matrix_means, num_features, num_features);
...
arma::mat posterior_mean_adjacency_threshold(arma::mat Adjacency_matrix_samples, double num_features, double threshold){
  ...
  arma::umat Adjacency_matrix_final_bool = (mean_matrix > threshold);
  arma::mat Adjacency_matrix_final = arma::conv_to<arma::mat>::from(Adjacency_matrix_final_bool);

  return Adjacency_matrix_final;
}

static inline double quantile_sorted_type7(const arma::vec& s, double q) {
  ...
  // Type 7 (R default): h = 1 + (n - 1) * q
  const double h = 1.0 + (static_cast<double>(n) - 1.0) * q; // in (1, n)
  const arma::uword k = static_cast<arma::uword>(std::floor(h));
  const double f = h - static_cast<double>(k);                // fractional part

  // Convert to 0-based indexing for Armadillo
  const arma::uword i = (k == 0) ? 0 : (k - 1);
  // Guard the upper neighbor
  const arma::uword ip1 = std::min(i + 1, n - 1);

  return s[i] + f * (s[ip1] - s[i]);
}

// [[Rcpp::export]]
arma::mat percentile_ci_causal_mat(const arma::mat& samples, double lower, double upper) {
  if (samples.n_rows == 0 || samples.n_cols == 0) {
    return arma::mat(0, 3, arma::fill::zeros);
  }
  if (!(lower >= 0.0 && lower <= 1.0) || !(upper >= 0.0 && upper <= 1.0) || !(lower <= upper)) {
    Rcpp::stop("lower/upper must satisfy 0 <= lower <= upper <= 1");
  }

  const arma::uword p = samples.n_cols;
  arma::mat ci(p, 3);

  for (arma::uword j = 0; j < p; ++j) {
    arma::vec col = arma::sort(samples.col(j));  // sort ascending
    ci(j, 0) = quantile_sorted_type7(col, lower);
    ci(j, 1) = quantile_sorted_type7(col, 0.5);
    ci(j, 2) = quantile_sorted_type7(col, upper);
  }
  return ci;
}

// [[Rcpp::export]]
List hpd_ci_mat(const arma::mat& Causal_effect_matrix_list, double cred_mass = 0.95) {

  double num_entries = Causal_effect_matrix_list.n_cols;
  double num_iter = Causal_effect_matrix_list.n_rows;
  double m = std::floor(num_iter * cred_mass);
  //Rcout << "m: " << m << std::endl;

  arma::colvec current_entry_list = Causal_effect_matrix_list.col(0);
  arma::mat hpd_interval_mat = arma::zeros<arma::mat>(num_entries, 2);

  for(int i = 0; i < num_entries; i++){
    current_entry_list = arma::sort(Causal_effect_matrix_list.col(i),"ascend");

    arma::vec distance_vec = arma::zeros<arma::vec>(num_iter - m);

    //Rcout << "num_entries - m: " << num_entries - m << std::endl;
    for(int j = 0; j < num_iter - m; j++){
      distance_vec[j] = current_entry_list[j+m] - current_entry_list[j];
    }

    double low_int_index = arma::index_min(distance_vec);
    double low_int = current_entry_list[low_int_index];
    double upper_int = current_entry_list[low_int_index+m];


    hpd_interval_mat(i,0) = low_int;
    hpd_interval_mat(i,1) = upper_int;
  }


  return List::create(Named("hpd_interval_mat") = hpd_interval_mat);
}
