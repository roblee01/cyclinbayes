...
generates_examples_DAG = function(num_covariates, N, M_input, prob_sparsity, seed_input){
  set.seed(seed_input)
  ...
  if (num_covariates<=10){
    repeat{
      ...
      if (gRbase::is.DAG(Causal_effect_matrix_true_str) && (det( identity_mat-Causal_effect_matrix_true_str) != 0) &&  (mean(Causal_effect_matrix_true_str) != 0)  ) break
    }#end of repeat
  }#end of if
